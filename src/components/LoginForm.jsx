    ...
    <div className="password-strength">
      <div className="strength-bar">
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
            className={`strength-segment${i <= level ? ` active ${key}` : ""}`}
          />
        ))}
      </div>
...
export default function LoginForm({ mode, onSwitch }) {
  const handleSubmit = (e) => e.preventDefault();

  return (
    <form className="auth-form" onSubmit={handleSubmit} noValidate>
      {mode === "login" ? (
        <LoginFields onSwitch={onSwitch} />
      ) : (
        <RegisterFields onSwitch={onSwitch} />
      )}
    </form>
  );
}
